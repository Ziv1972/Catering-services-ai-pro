...
function formatExtractedData(raw: string): string {
  try {
...
export default function AttachmentSection({
  ...
  const handleUpload = useCallback(async (files: File[]) => {
    ...
    try {
      await Promise.all(
        files.map((file) => attachmentsAPI.upload(entityType, entityId, file))
      );
  ...
  const handleDownload = async (att: AttachmentData) => {
    try {
      const blobData = await attachmentsAPI.download(att.id);
      const blob = blobData instanceof Blob
        ? blobData
        : new Blob([blobData], { type: att.content_type || 'application/octet-stream' });
  ...
  const handleProcess = async (attId: number, mode: 'summarize' | 'extract' | 'both') => {
    ...
    setError(null);
    try {
      const updated: AttachmentData = await attachmentsAPI.process(attId, mode);
      setAttachments((prev) =>
                    ...
                    {att.ai_extracted_data && (
                      <div>
                        ...
                        <pre className="text-xs text-gray-600 bg-white rounded p-2 overflow-x-auto max-h-48 border">
                          {formatExtractedData(att.ai_extracted_data)}
                        </pre>
                      </div>
                    )}
                    {/* Re-process buttons */}
